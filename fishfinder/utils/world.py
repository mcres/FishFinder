...
class Map():
    ...
    def __init__(self, conf, path_to_results):
        ...
        # distance per time
        self.avg_speed = conf["boat_features"]["avg_speed"]
        # volume per speed per distance
        self.fuel_consumption = conf["boat_features"]["fuel_consumption"]
        # volume per angle
        self.fuel_consumption_turning = conf["boat_features"]["fuel_consumption_turning"]
        # time
        self.time_consumption_turning = conf["boat_features"]["time_consumption_turning"]
    ...
    # (self.start_x, self.start_y, self.end_x, self.end_y, self.displacement):
    ...
    # (initial_pos, self.d1, self.d2, self.avg_speed):
    def fuel_prediction(self):
    ...
    # (grid_points, show_points, path_points, self.path_to_results):
    def show_grid(self):
    ...
    # (path_points_data, fish_list, self.path_to_results, boat_features):
